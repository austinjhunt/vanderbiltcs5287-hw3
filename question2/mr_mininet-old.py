...
def parseCmdLineArgs ():
    ...
    parser.add_argument ("-i", "--iters", type=int, default=5, help="iterations")
    parser.add_argument ("-f", "--metricsfile", type=str, default="metrics.csv", help="file to output metrics (csv)")
...
def runMapReduceWordCount (hosts, args):
    try:
        ...
        # Start the master
        print("Running cmd: python ./mr_wordcount.py -m ", str (args.map), " -r ", str (args.reduce), " -p ", str (args.masterport)," -i ", str(args.iters)," -f ", str(args.metricsfile), " ", args.datafile, " on host: ", hosts[0].IP())
        # popens [ master ] = master.popen ('ping', '-c3', master.IP() )
        popens [ hosts[0] ] = hosts[0].popen ('python', './mr_wordcount.py', '-m', str(args.map), '-r', str(args.reduce), '-p', str(args.masterport), " -i ", str(args.iters)," -f ", str(args.metricsfile), " ", args.datafile, stdout=subprocess.PIPE )
